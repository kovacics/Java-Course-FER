...
public class CopyShellCommand implements ShellCommand {
    ...
    public ShellStatus executeCommand(Environment env, String arguments) {
        ...
        File sourceFile = new File(parts[0]);
        File destinationFile = new File(parts[1]);
    ...
    private boolean wantToOverwrite(Environment env, String name) {
        ...
        if (env.readLine().trim().toUpperCase().equals("Y")) return true;
        if (env.readLine().trim().toUpperCase().equals("N")) {
